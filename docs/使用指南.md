...
**如果报「禁止运行脚本」**（`无法加载文件 ...，因为在此系统上禁止运行脚本`）：

```powershell
Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned
```
或者单次绕过：`powershell -ExecutionPolicy Bypass -File .\scripts\setup.ps1`

**没有 python？** `winget install Python.Python.3.12`，装完重开 PowerShell。
